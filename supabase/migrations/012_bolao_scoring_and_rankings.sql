create or replace function public.bolao_match_outcome(home_score integer, away_score integer)
returns integer
language sql
immutable
as $$
...
create or replace function public.recalculate_bolao_match_predictions(target_match_id uuid)
returns void
language plpgsql
security definer
...
begin
  ...
  set
    points_winner = case
      when public.bolao_match_outcome(p.home_score, p.away_score) = public.bolao_match_outcome(match_record.home_score, match_record.away_score)
...
create or replace function public.recalculate_bolao_predictions_on_match_finish()
returns trigger
language plpgsql
security definer
set search_path = public
...
begin
  ...
  then
    perform public.recalculate_bolao_match_predictions(new.id);
  end if;
...
create trigger recalculate_bolao_predictions_after_match_finish
after update of status, home_score, away_score on public.bolao_matches
for each row
execute function public.recalculate_bolao_predictions_on_match_finish();
